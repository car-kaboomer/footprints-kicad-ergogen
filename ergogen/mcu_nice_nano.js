...
//      if true, it will only place jumpers on the first 6 rows of pins, which can't be
//      reversed in firmware, i.e. RAW and P1, GND and P0, GND and RST, GND and VCC, and the next two pairs.
...
module.exports = {
  ...
  (segment (start ${p.eaxy(3.099695, -0.725 + (row_num * 2.54) - 12.7)}) (end ${p.eaxy(2.594695, -0.22 + (row_num * 2.54) - 12.7)}) (width 0.25) (layer "B.Cu"))
    ...
    const gen_traces = () => {
      let traces = '';
      for (let i = 0; i < 12; i++) {
        if (i < 6 || !p.only_required_jumpers) {
    ...
    const gen_socket_row = (row_num, pin_name_left, pin_name_right, show_via_labels, show_silk_labels) => {
      ...
      const net_silk_back_left = row_num < 6 ? via_label_right : via_label_left
      const net_silk_back_right = row_num < 6 ? via_label_left : via_label_right

      let socket_row_base = `
    ${''/* Socket Holes */}
    (pad "${socket_hole_num_left}" thru_hole circle (at -7.62 ${-12.7 + row_offset_y} ${p.r}) (size 1.7 1.7) (drill 1) (layers "*.Cu" "*.Mask") ${p.reversible && (row_num < 6 || !p.only_required_jumpers) ? p.local_net(socket_hole_num_left).str : net_left})
    (pad "${socket_hole_num_right}" thru_hole circle (at 7.62 ${-12.7 + row_offset_y} ${p.r}) (size 1.7 1.7) (drill 1) (layers "*.Cu" "*.Mask") ${p.reversible && (row_num < 6 || !p.only_required_jumpers) ? p.local_net(socket_hole_num_right).str : net_right})
      `
      let socket_row_vias = `
    ${''/* Inside VIAS */}
    ...
    (pad "${socket_hole_num_right}" smd custom (at 5.5 ${-12.7 + row_offset_y} ${180 + p.r}) (size 0.2 0.2) (layers "B.Cu" "B.Paste" "B.Mask") ${p.local_net(socket_hole_num_right).str}
      ...
      if (p.reversible && (row_num < 6 || !p.only_required_jumpers)) {
      ...
      if (show_silk_labels == true) {
        if (p.reversible || p.show_silk_labels_on_both_sides || p.side == 'F') {
          ...
          if (row_num != 9
            ...
            socket_row += `
    (fp_text user "${net_silk_front_left}" (at -${p.reversible && (row_num < 6 || !p.only_required_jumpers) ? (net_silk_front_left.length > 2 ? 1.45 : 2.04) : 4.47} ${-12.7 + row_offset_y} ${p.r}) (layer "F.SilkS")
      (effects (font (size 1 1) (thickness 0.15)))
          ...
          if (row_num != 9 
            ...
            socket_row += `
    (fp_text user "${net_silk_front_right}" (at ${p.reversible && (row_num < 6 || !p.only_required_jumpers) ? (net_silk_front_right.length > 2 ? 1.45 : 2.04) : 4.47} ${-12.7 + row_offset_y} ${p.r}) (layer "F.SilkS")
      (effects (font (size 1 1) (thickness 0.15)))
    )
            `
          }
        ...
        if (p.reversible|| p.show_silk_labels_on_both_sides || p.side == 'B') {
          ...
          if (row_num != 9 
            ...
            socket_row += `
    (fp_text user "${net_silk_back_left}" (at ${p.reversible ? '-' : ''}${p.reversible && (row_num < 6 || !p.only_required_jumpers) ? (net_silk_back_left.length > 2 ? 1.45 : 2.04) : 4.47} ${-12.7 + row_offset_y} ${p.r}) (layer "B.SilkS")
      (effects (font (size 1 1) (thickness 0.15)) (justify mirror))
          ...
          if (row_num != 9 
            ...
            socket_row += `
    (fp_text user "${net_silk_back_right}" (at ${p.reversible ? '' : '-'}${p.reversible && (row_num < 6 || !p.only_required_jumpers) ? (net_silk_back_right.length > 2 ? 1.45 : 2.04) : 4.47} ${-12.7 + row_offset_y} ${p.r}) (layer "B.SilkS")
      (effects (font (size 1 1) (thickness 0.15)) (justify mirror))
    )
            `
          }
      ...
      if (show_via_labels && (p.reversible && (row_num < 6 || !p.only_required_jumpers))) {
        socket_row += `
    ${''/* Via Labels - Front */}
